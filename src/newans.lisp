...
(DEFUN ANSCOMMAND (RSS) 
       ...
       (PROG (EXP ANS SUCCESS PLAN PLAN2) 
	     ...
	     (SETF (GET RSS 'PLNRCODE=) EXP)			       ;APPROPRIATE PLANNER THANDS AND THORS.
	     (SETQ EXP (AMBPUT EXP))
...
(DEFUN ANSNAME (PHRASE) 
       ...
       (PROG (ANSNODE C N CUT SM-TIME MP-TIME) 
	     ...
	     (SETF (GET (CAR (SM ANSNODE)) 'REFER=)
		      (CADR PHRASE))))				       ; PUT THE REFERENT ON AS THE GUY GIVEN BY ANSWER
...
(DEFUN ANSREL (RSS) 
       ...
       (PROG (TYPE REL CODE PLAUS ANS PHRASE LENGTH NUM) 
	     ...
	     (AND (EQ TYPE 'ALL)
		  (SETF (GET RSS 'NEGATIVE=) T))		       ;"SAY" PHRASES WHICH THE OTHER PARTS GENERATE.
	     (SETF (GET RSS 'PLNRCODE=)				       ;UNIVERSALS ARE CONVERTED TO NOT THERE EXISTS NOT.
	      (SETQ 
	       CODE
	       (PLNR-FINDIFY 'ALL
			     (VARIABLE? REL)
		...
		(ANSBUILD (+ PLAUS (PLAUSIBILITY? RSS))
		 ...
		 (COND
		  ...
		  ((NUMBERP NUM)
		   (APPEND (COND ((EQ NUM LENGTH)
				  '((SAY YES\,)))
				 ((> LENGTH NUM) NIL)		       ;THIS IS THE CASE WHERE WE ARE CAGEY AND AVOID
...
(DEFUN DISPUT (ASSERTION) 
       ...
       (OR (NOT DISCOURSE) (SETF (GETF (CDR ASSERTION) 'WHO) SENTNO)))
...
(SETF (GET 'TC-NAMEOBJ 'THEOREM)
	 '(THCONSE ((X ITEM) TYPE COLOR NAME SIZE Y Z)		       ; PLANNER IS CALLED TO SEE HOW MANY OBJECTS FIT
		  ...
		  (OR (SETQ CUBE (AND (EQ (THV TYPE) '\#BLOCK)	       ;FORCED TO MATCH THE CHOSEN ITEM  THIS IS VALUE
				      (\#EQDIM (THV X))))		       ;IS THE ENTIRE TYPE ASSERTION FOR
		      T)					       ;SPECIAL CHECK TO CALL EQUIDIMENSIONAL BLOCKS
		  (THCOND ((THGOAL (\#NAME (THV X) (THV NAME)))		       ;"CUBE" THE OR IS TO PREVENT PLANNER FROM
			   (SETQ NAME THVALUE))			       ;FAILING THE CHOSEN OBJECT.  IT IS SAVED SO THE
			  ((THGOAL (\#IS (THV Y) (THV TYPE)))		       ;SENTENCE NUMBER CAN BE PUT ON ITS PROPERTY LIST
			   (OR (NOT CUBE) (\#EQDIM (THV Y)))	       ;IF THE FACT IS USED IN THE DESCRIPTION.  IF THE
			   (SETQ TYPELIST (CONS (THV Y) TYPELIST))	       ;ITEM HAS A NAME, NO MORE IS NEEDED. FIND
			   ...
			   (THFAIL)))))				       ;COLOR WE DON'T WANT TO CHECK FOR EXACT EQUALITY
	 							       ;OF SIZE, JUST WHETHER THEY WOULD BE CALLED THE
...
(DEFUN THVAL-MULT (CODE) 
       ...
       (PROG (ANS) 
	     ...
	     (RETURN (COND ((EQUAL ANS
				   (THVAL2 (LIST (- SENTNO 2.)         ;ANSWERED IT AT ALL.
						 (+ 1 SENTNO))
					   CODE))
       ...
       (PROG (HAVE NEED XX ANS PLNRCODE LOOP) 
		...
		(MAPBLAND '(LAMBDA (OSS) 
	     ...
	     (SETQ PLNRCODE (PLNR-DESCRIBE (RELATIONS? OSS)
					   (VARIABLE? OSS)
					   (LIST (VARIABLE? OSS))))
	     (SETF (GET OSS 'PLNRCODE=) PLNRCODE)
	     (COND
