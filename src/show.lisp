...
(DEFUN-FEXPR ABBREVIATE (A) 
       (MAPCAR #'(LAMBDA (X) 
			(SETF (GET (READLIST (MAPCAR #'(LAMBDA (X Y) X)
						   (EXPLODE X)
						   '(T T))) 'ABBREV)
				 X))
	       A)
...
(DEFUN SHOWTELL (A NODE SYSTEMS INFO ACTION) 
       ...
       (COND ((NULL A) (SHOWTELLCHOICE))
	     ((GET (CAR A) ACTION)
	      (APPLY (EVAL (GET (CAR A) ACTION)) (LIST A)))
...
(DEFUN SHOWTELLCHOICE NIL 
       (APPLY (EVAL (GET (SETQ NODE (QUERY '(WHICH OPTION?)
...
(DEFUN QUERY (TEXT CHOICES HELP) 
	...
	TOP  (SETQ CH2 (CONS 'QUIT CHOICES) EX2 EXPL)
	     (PRINTEXT TEXT)
	READ (COND ((MEMBER (SETQ CHAR (CHAR-UPCASE (READ-CHAR))) BREAKCHARS)
	     ...
	     (MAPC #'(LAMBDA (X Y) (AND (CHAR= CHAR (CHARACTER (CAR X)))
				       (SETQ CH3 (CONS Y CH3))
	     ...
	     (AND CH3
		  ...
		  (SETQ NOTINIT T)
		  (GO READ))
	GO   (OR (MEMBER (READ-CHAR) BREAKCHARS) (GO GO))
	CHOICES
...
(DEFUN DEFINETHEOREM (X) 
       (SETF (GET (COND ((CDR X) (SETQ X (CADR X)))
...
(DEFUN-FEXPR DEFINE (A) 
	...
	MAR  (OR (SETQ MARK (REQUEST 'MARKERS\:
	     ...
	     (COND
	      ((EQ TYPE 'NOUN)
	       (SETF (GET A 'FEATURES) '(NOUN NS))
	       (SETF (GET A 'SEMANTICS)
		(LIST
		 (LIST
		  'NOUN
		  (LIST
