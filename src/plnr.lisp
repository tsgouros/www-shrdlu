; This file is supposed to form an "automated theorem prover".  I
; do not fully understand it, and all attempts at getting it to
; work were made in an ad hoc manner.
;
;	Keldon (09/19/1999)
...
(DEFUN-FEXPR THAMONG
  ...
  (COND						;$E - (THAMONG $E$?X (THFIND ... )) CAUSES THE
    ...
    ((EQ (CADR (SETQ THXX (THGAL (COND ((EQ (CAAR THA)	;OLD BINDING CELL OF $?X (OR $E$?X) IF $?X
					    ...
					    'THEV)	;THUNASSIGNED, OLD VALUE AND LIST OF NEW 	
					(THVAL (CADAR THA)	;THAMONGF.
					       THALIST))
				       (T (CAR THA)))
				 THALIST)))
	 'THUNASSIGNED)
     (THPUSH THTREE (LIST 'THAMONG
			  THXX
...
(DEFUN-FEXPR THANTE
       ...
       (THDEF 'THANTE THX))					       ;THEOREMS)

(DEFUN-FEXPR THAPPLY (L) (THAPPLY1 (CAR L)
...
(DEFUN THASS1
 ...
 (PROG (THX THY1 THY TYPE PSEUDO)
       (DECLARE (SPECIAL THX THY TYPE))
       (AND (CDR THA) (EQ (CAADR THA) 'THPSEUDO) (SETQ PSEUDO
							      T))
       ...
       (AND THTRACE (NOT PSEUDO) (THTRACES (COND (P 'THASSERT)
						 ('THERASE))
					   THX))
...
(DEFUN-FEXPR THASSERT (THA) (THASS1 THA T))		;THASS1 IS USED FOR BOTH ASSERTING AND ERASING,
...
(DEFUN THASSERTT NIL (PROG1 (CADAR THTREE) (THPOPT)))
...
(DEFUN THBI1 (X) (COND ((ATOM X) (LIST X 'THUNASSIGNED))
		       (T (LIST (CAR X) (EVAL (CADR X))))))

(DEFUN-FEXPR THBKPT (L) (OR (AND THTRACE (THTRACES 'THBKPT L))
			    THVALUE))
...
(DEFUN-FEXPR THCONSE (THX) (THDEF 'THCONSE THX))
...
(DEFUN-FEXPR THDO
       (A)
       (OR (NOT A)
	   (PROGN (THPUSH THTREE (LIST 'THDO A NIL NIL))
		  (SETQ THEXP (CAR A)))))
...
(DEFUN-FEXPR THERASE (THA) (THASS1 THA NIL))
...
(DEFUN THERASET NIL (PROG1 (CADAR THTREE) (THPOPT)))

; (COMMENT THERASING DEFINES AND OPTIONALLY ASSERTS ERASING THEOREMS)

(DEFUN-FEXPR THERASING (THX) (THDEF 'THERASING THX))

(DEFUN-FEXPR THFAIL
       (THA)
       (AND THA
	    (PROG (THTREE1 THA1 THX)
	     F	  (SETQ THA1 (COND ((EQ (CAR THA) 'THEOREM)
				    'THPROG)
				   ((EQ (CAR THA) 'THTAG)
				    'THPROG)
				   ((EQ (CAR THA) 'THINF)
 ...
 (THPUSH THTREE
	 (LIST 'THFIND
	       (COND ((EQ (CAR THA) 'ALL) ' (1. NIL NIL))	       ;STANDARD ALL
		     ...
		     ((EQ (CAAR THA) 'EXACTLY)
		      (LIST (CADAR THA) (+ 1 (CADAR THA)) NIL))
		     ((EQ (CAAR THA) 'AT-MOST)
		      (LIST 1. (+ 1 (CADAR THA)) NIL))
...
(DEFUN THFINDF
       NIL
       (SETQ THBRANCH NIL)
       (COND ((OR THMESSAGE (< (CAADR (SETQ THXX (CDAR THTREE)))
				   (CAAR THXX)))
	      (THPOPT)
	      NIL)
	     (T (THPOPT) (CDADR THXX))))
...
(DEFUN THFINDT
       ...
       (PROG (THX THY THZ THCDAR)
	     ...
	     (AND (EQ (SETQ THY (+ 1 (CAADR THCDAR))) (CADAR THCDAR))
...
(DEFUN-FEXPR THFLUSH						       ;(THFLUSH) FLUSHES ALL ASSERTIONS AND THEOREMS
       ...
       (COND ((OR (NULL THA1)					       ;SHOULD DATA BASE BE SEARCHED TRYED IF NO RECS
		  (AND (NOT (AND (EQ (CAAR THA1) 'THANUM)
		       ...
		       (NOT (AND (EQ (CAAR THA1) 'THNODB)	       ;TRIED IF REC NOT THNODB OR (THDBF PRED)
				 (PROGN (SETQ THA1 (CDR THA1)) T)))
		       (NOT (EQ (CAAR THA1) 'THDBF))))
	      (SETQ THA1
		    (CONS (LIST 'THDBF 'THTRUE) THA1))))
       (SETQ THA1 (MAPCAN #'THTRY THA1))		       ;THMS AND ASSERTIONS SATISFYING RECS APPENDED TO
      (AND THTRACE (THTRACES 'THGOAL THA2))			       ;RECS
...
(DEFUN THGOALF
       ...
       (COND (THMESSAGE (THPOPT) NIL) ((THTRY1)) (T (THPOPT) 
NIL)))

(DEFUN THGOALT NIL (PROG1
...
;; THT1 has just been set to our database of all assertions
;; about THI1.  If THADD added "(\#IS :B1 \#BLOCK)"
;; It'd send \#IS to THIP, and THIP here would find all IS-assertions
;; And then it looks for a match
;; by the way, the actual command would be:
;; (GET '\#IS 'THASSERTION)
		   ...
		   ((AND (OR THFST THFSTP)
			 ...
			 (COND ((EQ THWH 'THASSERTION)
				(ASSOC THTT (CDDR THT3) :TEST #'EQUAL))
			       ...
			       (T (MEMBER THTT (CDDR THT3) :TEST #'EQUAL))))
...
;; IF you THADD, this is where a new assertion is actually tacked on
		    ;;HACK IN THE LATEST ENTRY INTO THE SUB-SUB-BUCKET
		    (RPLACA (CDR THT3) (+ 1 (CADR THT3)))
...
(DEFUN THMATCH2
 ...
 (AND (LISTP THX) (EQ (CAR THX) 'THEV)
      (SETQ THX (THVAL (CADR THX) THOLIST)))
 (AND (LISTP THY) (EQ (CAR THY) 'THEV)
 ...
 (COND
  ...
  ((OR (AND (LISTP THX) (MEMQ (CAR THX) '(THV THNV THRESTRICT)))
       (AND (LISTP THY) (MEMQ (CAR THY) '(THV THNV THRESTRICT))))
   ((LAMBDA (XPAIR YPAIR)
    ...
    (COND ;;IF THX IS A NORMAL VARIALBE, IN PARTICULAR
	  ...
	  ;;BINDING LIST
	  ((AND (LISTP THX) (EQ (CAR THX) 'THRESTRICT))
...
(DEFUN THMATCH1
       ...
       (PROG (THML)
	     ...
	     (COND ((AND (EQ (LENGTH (COND ((EQ (CAR THX)
			 ...
			 (CATCH 'NO-MATCH (MAPC #'THMATCH2 THX THY)))
...
(DEFUN THMATCHLIST
	...
	THP2 (COND 
		   ...
		   ((> THL THRN) (SETQ THL1 THA1)
					(SETQ THL THRN)))
	     
	     ;;GO BACK FOR ANOTHER PASS
	     (GO THP1)))

(DEFUN-FEXPR THMESSAGE
       (THA)
...
(DEFUN THNOFAIL (THX) (COND (THX (SETF (GET 'THPROG 'THFAIL) 'THPROGT))
			    (T (SET (GET 'THPROG 'THFAIL) 'THPROGF))))

(DEFUN-FEXPR THNOHASH
       (THA)
       (MAPC #'(LAMBDA (X) (SETF (GET (CAR THA) X)
				    'THNOHASH))
	     (OR (CDR THA)
		 '(THASSERTION THCONSE THANTE THERASING))))

(DEFUN-FEXPR THNOT (THA) (SETQ THEXP
			       (LIST 'THCOND
				     (LIST (CAR THA)
					   '(THFAIL THAND))
				     '((THSUCCEED)))))

(DEFUN-FEXPR THNV (X) (THV1 (CAR X)))

(DEFUN-FEXPR THOR (THA) (AND THA
...
(DEFUN THOR2 (P) (COND (THMESSAGE (THPOPT) NIL)
			...
			(RPLACA (CDAR THTREE) (CDADAR THTREE))
			(SETQ THEXP (COND (P (PROG1 (CAADAR THTREE)
						    (OR (CADAR THTREE)
							(THPOPT))))
					  ((CAR (CAADAR THTREE))))))
		       (T (THPOPT) NIL)))
...
(DEFUN THPROGA
       ...
       ((LAMBDA (X) (COND 
			  ...
			  ;;RETURN SUCCESS
;			  ((NULL (CDAR X)) (THPOPT) NIL)
; No idea why anyone would have wanted to code the above.
; NIL would not indicate SUCCESS, as the above comment suggests.
...
(DEFUN THPURE
       ...
       (CATCH 'VAR-EXIST
              (MAPC #'(LAMBDA (Y) (AND (THVAR Y) (THROW 'VAR-EXIST NIL)))
		     XX)))
...
(DEFUN THREMOVE
       ...
       (PROG (THB1 THWH THNF THAL THON THBS THFST THFSTP THFOO)
             (DECLARE (SPECIAL THWH THNF THAL THON THBS THFST THFSTP))
...
(DEFUN-FEXPR THRETURN (X) (APPLY-THSUCCEED
				 (CONS 'THPROG X)))
...
(DEFUN-FEXPR THURPLACA (L) (RPLACA (CAR L) (CADR L)))
...
(DEFUN-FEXPR THURPLACD (L) (RPLACD (CAR L) (CADR L)))
...
(DEFUN-FEXPR THSTATE
 ...
 (PROG (THP)							       ;;EITHER NUMBERS, LIST STRUCTURE, NOHASHED ATOMS
       (PRINT '(THDATA))					       ;OR NON-INTERNED ATOMS.
       (MAPC
	 #'(LAMBDA (BUCKET)
	  (MAPC
	    #'(LAMBDA (THATOM)
 ...
 (COND
  ...
  ((EQ (CAR XX) 'THUSE)
   (MAPCAR
    ...
    (CDR XX)))
  ((EQ (CAR XX) 'THTBF)
   (MAPCAN #'(LAMBDA (Y) (COND ((APPLY (CADR XX) Y)
					(LIST (LIST 'THAPPLY
						    Y
						    (CAR THX))))))
...
(DEFUN THTRY1							       ;TRIES NEXT RECOMMENDATION ON TREE FOR THGOAL
       ...
       (PROG (THX THY THZ THW THEOREM)
	     ...
	     (RPLACD THY (- (CDR THY) 1))
	NXTREC
	     (COND ((OR (NULL (CAR THY)) (ZEROP (CDR THY)))
		    (RETURN NIL)))				       ;RECOMMENDATIONS EXHAUSTED. FAIL
	     (SETQ THX (CAAR THY))
             (COND ((EQ (CAR THX) 'THNUM) (GO THNUM))
	           ((EQ (CAR THX) 'THDBF) (GO THDBF))
		   ((EQ (CAR THX) 'THTBF) (GO THTBF))
		   ((EQ (CAR THX) 'THTBF1) (GO THTBF1))
		   (T (ERT THTRY1 BAD TAG)))
...
(DEFUN THVAL
       ...
       (DECLARE (SPECIAL THEXP THALIST))
       
       ;;ALL THPUSH DOES IS TO CONSE ON THE SSECOND ITEM TO THE FIRST
       (THPUSH THLEVEL (LIST THTREE THALIST))
       (PROG (THTREE THBRANCH THOLIST THABRANCH THE THMESSAGE THE2)
             (DECLARE (SPECIAL THTREE THOLIST THMESSAGE
	                       THBRANCH THABRANCH))
...
(DEFUN THVAR
       ...
       (AND (LISTP X) (MEMQ (CAR X) '(THV THNV))))
...
; This used the be the function "THINIT", but now we just
; execute the necessary steps at the top level.
(SETQ THGENAME 0.)
(SETQ THSTEP NIL)
(SETQ THSTEPD NIL)
(SETQ THSTEPT NIL)
