...
(DEFUN ADD-F-PT (FEATURE PTR) 
       (SETF (GET (CAR PTR) 'FEATURES) (CONS FEATURE (FE PTR)))
       (AND (EQ PTR C) (SETQ FE (FE PTR)))
       T) 

(DEFUN REMOVE-F-PT (FEATURE PTR) 
       (SETF (GET (CAR PTR) 'FEATURES)
...

(DEFUN-FEXPR CALLSM (SEMANTIC-EXPRESSION) 
...
(DEFUN PARSE2 (REST P) 
       ...
       (PROG (UNIT CREATED-NODE END PARENT RE SPECIAL NBB) 
             (DECLARE (SPECIAL PARENT RE SPECIAL))
	     ...
	     (COND ((NULL (SETQ RE (APPLY-GRAMMAR UNIT)))	       ;THIS IS WHERE ALL THE WORK HAPPENS. IF THE
		    (SETQ RE NIL)				       ;PARSE SUCCEEDS, IT WILL RETURN THE NODE THAT HAS
...
(DEFUN-FEXPR POPTO (A) 
	...
	LOOP (COND ((EVAL (CONS 'ISQ (CONS 'XX A))))
		   ...
		   ((MQ POPTO) (RETURN NIL)))
	EX   (COND ((EQ XX H) (RETURN C)) ((SHRDLU-POP) (GO EX))))) 
...
(DEFUN SETR (REGISTER VALUE NODE) 
       ...
       (SETF (GET (CAR NODE) REGISTER) VALUE)) 
...
(DEFUN UPCHECK
	 NIL (AND (MOVE-PT C U (REL-NOT-FOUND))
			  (NOT (MEET (FE PT)
				     '(OBJ1Q OBJ1REL
					     OBJ2Q
					     OBJ2REL
