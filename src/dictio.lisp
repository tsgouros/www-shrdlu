...
(DEFS \#DEFINE 
    EXPR (LAMBDA (X Y) 
		 (LIST '\#DEFINITION
		       ...
		       (PROG (X) 
			     (SETF (GETF (SETQ X (MAKESYM 'ATM)) 'NEWWORD)
				      (INTERP Y))
			     (RETURN X)))))

(DEFS \#DEFINITION 
    FEXPR (LAMBDA (A) 
	   (SETF (GET (CADAR A) 'WORD) '(NOUN NS))
	   (SETF (GET (CADAR A) 'SMNTC)
	    (SUBST (SUBST '***
...
(DEFS \#PROPDEFINE 
    EXPR (LAMBDA (X) 
		 (SETF (GET X 'FEATURES) '(PROPN NS))		       ;CHANGED TO FEATURES FROM 'WORD' IN THE OLD
		 (SETF (GET X 'SEMANTICS) '((PROPN T)))))		       ;DICTIONARY
